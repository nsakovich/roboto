...
var Crawler = function(options) {
  ...
  this._downloader = require('./downloaders/default')(this.options);
...
Crawler.prototype._crawlNext = function(done) {
  ...
  var nextLink = self._urlFrontier.getNextLink( function(link){
    ...
    if (link.referer && link.referer.href) {
      domain = self.botParser._getDomainFromHref(link.referer.href);
    }

    if (!domain || domain === opts.requiredReferedDomain) {
      self.emit('next', link);
...
Crawler.prototype._crawlPage = function(link, done) {
  ...
  link.path = this._createPath(link);
...
Crawler.prototype._createPath = function(link) {
  var path = [];
  var current = link;

  while(current) {
    if (current.referer instanceof Object) {
      current = current.referer;
      path.unshift(current.href);
    }
    else {
      current = null;
    }
  ...
  return function(done) {
    ...
    var handler = function(err, response, body) {
      ...
      if (statusCode >= 400) {
        ...
        var res = _.extend(response, { url: href, urlPath: path });
        ...
        this.emit('httpSuccess', href, res, link.referer.href);
...
Crawler.prototype._parse = function(currentLink, response, done) {
  ...
  if (currentLink.href) {
    domain = this.botParser._getDomainFromHref(currentLink.href);
  }
