...
LinkExtractor.prototype._followable = function(link) {
  ...
  if (rel && rel.indexOf('nofollow') > -1 && this.options.obeyNofollow) {
...
LinkExtractor.prototype.normalizeUrl = function(link, response) {
  ...
  //if queryParams allowed, include those too.
  if (this.options.allowQueryParams) {
    resultParts.search = hrefInfo.search || requestInfo.search;
  }

  if (this.options.allowHashParam) {
    var hash = hrefInfo.hash;// || requestInfo.hash;
    if (hash && hash !== "#") {
      resultParts.hash = hash;
    }
  }
